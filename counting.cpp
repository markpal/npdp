...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5) schedule(dynamic, 1)
...
for( c1 = max(0, floord(l - 2, 8) - 1); c1 <= floord(N - 3, 8); c1 += 1)
  ...
  for( c1 = c0 - (c0 + 17) / 17 + 1; c1 <= min(min(N - 3, c0), c0 + (-l + 1)/16 + 1); c1 += 1)
    ...
    if(kind==5)  // dapt     code checked
    {
        printf("DAPT\n");
        for (int w0 = max(-1, floord(l + 1, 16) - 1); w0 < floord(N, 16); w0 += 1) {
          #pragma omp parallel for schedule(dynamic, 1)
          for (int h0 = max(max(w0 - (N + 16) / 16 + 1, -((N + 13) / 16)), -((N - l + 15) / 16)); h0 < 0; h0 += 1) {
            for (int i0 = max(max(max(max(-N + 2, -N + l), l - 16 * w0 + 16 * h0 - 15), -16 * w0 + 16 * h0 - 13), 16 * h0); i0 <= 16 * h0 + 15; i0 += 1) {
              for (int i1 = max(max(16 * w0 - 16 * h0, l - i0), -i0 + 2); i1 <= min(N, 16 * w0 - 16 * h0 + 15); i1 += 1) {
                for (int i2 = -i0; i2 <= -l + i1; i2 += 1) {
                  c[-i0][i1] += ((c[-i0][i1 - 1] + paired((i2), (i1))) ? (c[-i0][i2 - 1] + c[i2 + 1][i1 - 1]) : 0);
                }
              }
            }
          }
